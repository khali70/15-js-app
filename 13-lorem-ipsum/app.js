...
const artical = document.querySelector(".lorem-text");
//console trace
form.addEventListener("submit", (e) => {
  ...
  if (isNaN(value) || value < 0 || value > 9) {
    window.confirm("invalid number would you like to have 1 lorem ipson")
      ? (artical.innerHTML = `<p class="result">${text[ran()]}</p>`)
      : null;
  } else {
    for (let looper = value - 1; looper >= 0; looper--) {
      let random = Math.floor(Math.random() * arr.length);
      artical.innerHTML += `<p class="result">${arr.splice(random, 1)}</p>`;
    }
  }
});
